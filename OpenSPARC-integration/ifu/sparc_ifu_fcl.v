...
module sparc_ifu_fcl(/*AUTOARG*/
   ...
   dyser_ifu_stallreq, ifu_dyser_rollback_e, ifu_dyser_rollback_w,
   // **
...
//               dec_fcl_kill4sta_e;
   ...
   output      ifu_dyser_rollback_e;
   output      ifu_dyser_rollback_w;
   // **
...
//   assign async_rst_i2 = tlu_ifu_rstthr_i2  & {4{tlu_ifu_rstint_i2}} &
   ...
   //** DySER modification
   //dff_s #(1) dyser_stlreq_reg(.din (dyser_ifu_stallreq),
   //                            .q   (dyser_ifu_stallreq_d1),
   //                            .clk (clk), .se(se), .si(), .so());
   //**
...
//                      | thr_match_ne & dtu_fcl_ntr_s & retract_iferr_d1;
   ...
   // ** DySER modification
   // We need to rollback d and e stage instructions to prevent it goes to
   // m-stage when dyser requests stall.
   // However, stall signal will still be there and we don't want to 
   // always rollback. As a result we need a state machine here
   wire dyser_stall_state;

   dffrl_s dyser_stall_state_ff(.din (dyser_ifu_stallreq),
	                        .q   (dyser_stall_state),
                                .clk (clk),
                                .rst_l (fcl_reset_l),
                	        .se  (se), .si(), .so());

   wire rb2_inst_e_orig; // original sparc signal
   ...
   // rollback 2 instruction at e stage
   assign rb2_inst_e = rb2_inst_e_orig | (dyser_ifu_stallreq & ~dyser_stall_state);
   ...
   // rollback 1 instruction at d stage
   wire rb1_inst_d_orig; // original sparc signal
   ...
   // rollback 1 instruction at d stage
   assign rb1_inst_d = rb1_inst_d_orig | (dyser_ifu_stallreq & ~dyser_stall_state);
   // **
   ...
   //** DySER modification
   // If rollback from e stage, kill e stage's dyser instruction
   assign ifu_dyser_rollback_e = ((rb2_inst_e_orig | rt2_inst_e) & (inst_vld_e | intr_vld_e) & thr_e[0])|
                                  ~inst_vld_e  | // & ~dtu_inst_anull_e & 
	                           kill_curr_e |
                                   // rb_stg_e
                                   mark4rb_e;
   assign ifu_dyser_rollback_w = rb_stg_w;
   //** 
